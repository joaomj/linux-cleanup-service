...
def load_config() -> Config:
    ...
    state_dir = config_dir if is_macos else Path(os.environ.get("XDG_STATE_HOME", home / ".local" / "state")) / SERVICE_NAME
    data_dir = Path(os.environ.get("XDG_DATA_HOME", home / ".local" / "share")) / "opencode"
    cache_dir = Path(os.environ.get("XDG_CACHE_HOME", home / "Library" / "Caches" if is_macos else home / ".cache"))
    raw_backup_dir = _value(values, "OPENCODE_BACKUP_DIR", "")
    if raw_backup_dir:
        backup_dir = Path(raw_backup_dir).expanduser()
    else:
        legacy_backup_dir = home / "backups" / "opencode"
        backup_dir = legacy_backup_dir if legacy_backup_dir.is_dir() else state_dir / "backups"
